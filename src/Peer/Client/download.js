...
module.exports =async (torrent, sharedPieceBuffer, sharedReceivedBuffer, sharedRequestedBuffer, sharedFreqBuffer, fileInfoList, state,win) => {
  ...
  let download = (peers) =>{
    ...
    function runNextWorker() {
      ...
      const worker = new Worker(path.resolve(__dirname, '../../../src/Peer/Client/downloadWorker.js'), 
        {workerData: 
          { torrent, 
      ...
      worker.on('message', (msg) => {
        if(msg.type== 'have'){
          ...
          updateProgressList(torrent, msg.payload,fileInfoList,win);
        }
          ...
          updateNumPeerConnected(torrent, connectedPeer[inforHash(torrent)].length,win);
        }
  ...
  countSourceTimer = setInterval(() => {
    ...
    updateNumPeerDownloading(torrent, count,win);
  },50);
